...
class Sim(pygiv.ClassViewObj):
    ...
    def __init__(self):
        ...
        self.SetTags(
            "HiddenInhibGi",
            'def:"1.8" desc:"hidden layer inhibition -- increase to make sparser"',
        )
        self.WtInitVar = float(0.25)
        self.SetTags(
            "WtInitVar",
            'def:"0.25" desc:"random initial weight variance -- increase to make more random"',
        )
        self.FmContext = float(1)
        self.SetTags(
            "FmContext", 'def:"1" desc:"relative WtScale.Rel from Context layer"'
        )
        self.XCalLLrn = float(0.0003)
        self.SetTags(
            "XCalLLrn",
            'min:"0" step:"0.0001" def:"0.0003" desc:"amount of Hebbian BCM learning based on AvgL long-term average activity -- increase to increase amount of hebbian"',
        )
        ...
        self.SetTags(
            "ABPats", 'view:"no-inline" desc:"AB paired associate training patterns"'
        )
        self.ACPats = etable.Table()
        self.SetTags(
            "ACPats", 'view:"no-inline" desc:"AC paired associate training patterns"'
        )
        ...
        self.TestUpdate = leabra.TimeScales.Quarter
        self.SetTags(
            "TestUpdate",
    ...
    def ConfigEnv(ss):
        ...
        ss.TrainEnv.Table = etable.NewIndexView(ss.ABPats)
        ...
        ss.TestEnv.Table = etable.NewIndexView(ss.ABPats)
        ss.TestEnv.Validate()
        ss.TestEnv.Run.Max = (
            ss.MaxRuns
        )  # note: we are not setting epoch max -- do that manually
    ...
    def TrainTrial(ss):
        ...
        if chg:
            ...
            if ss.TrainEnv.Table.Table.MetaData["name"] == "AB" and (
                learned or epc == 50
            ):
                ss.TrainEnv.Table = etable.NewIndexView(ss.ACPats)
                learned = False
    ...
    def TestAll(ss):
        ...
        if not ss.StopNow:
            ss.TestNm = "AC"
            ss.TestEnv.Table = etable.NewIndexView(ss.ACPats)
            ss.TestEnv.Init(ss.TrainEnv.Run.Cur)
            while True:
                ss.TestTrial(True)
                chg = env.CounterChg(ss.TestEnv, env.Epoch)
    ...
    def RepsAnalysis(ss):
        ...
        ss.HiddenReps.SimMat.TableColStd(
            rels, "Hidden", "TrialName", True, metric.Correlation
        )
        ss.HiddenReps.PCA.TableColStd(rels, "Hidden", metric.Covariance)
        ss.HiddenReps.PCA.ProjectColToTable(
            ss.HiddenReps.PCAPrjn, rels, "Hidden", "TrialName", go.Slice_int([0, 1])
        )
    ...
    def LogRun(ss, dt):
        ...
        tstlog = ss.TstEpcLog
        tstix = etable.NewIndexView(tstlog)
        # compute mean over last N epochs for run level
        if nlast > tstix.Len() - 1:
            nlast = tstix.Len() - 1
        tstix.Indexes = tstix.Indexes[tstix.Len() - nlast :]

        params = "hid_gi: %g, wt_var: %g, fm_ctxt: %g, lrate: %g" % (
            ss.HiddenInhibGi,
            ss.WtInitVar,
            ss.FmContext,
            ss.Lrate,
