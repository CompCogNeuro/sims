...
class Sim(pyviews.ClassViewObj):
    ...
    def __init__(self):
        ...
        self.SetTags(
            "TestingEnv",
            'desc:"the environment to use for testing -- only takes effect for TestAll"',
        )
        ...
        self.SetTags(
            "ProbePats",
            'view:"no-inline" desc:"testing patterns based on training data"',
        )
        ...
        self.SetTags(
            "PhonConsPats", 'view:"no-inline" desc:"phonology consonant patterns"'
        )
        self.PhonVowelPats = etable.Table()
        self.SetTags(
            "PhonVowelPats", 'view:"no-inline" desc:"phonology vowel patterns"'
        )
        ...
        self.SetTags(
            "TstErrLog",
            'view:"no-inline" desc:"testing trials with errors (aggregating over multiple locations)"',
        )
        self.TstRTStats = etable.Table()
        self.SetTags(
            "TstRTStats", 'view:"no-inline" desc:"average RT as a function of type"'
        )
        ...
        self.SetTags(
            "TrainEnv",
            'desc:"Training environment (frequency-based) -- contains everything about iterating over input / output patterns over training"',
        )
        ...
        self.TrlPhon = str()
        self.SetTags(
            "TrlPhon", 'inactive:"+" desc:"pronunciation from phonology pattern"'
        )
        self.TrlPhonSSE = float()
        self.SetTags(
            "TrlPhonSSE",
            'inactive:"+" desc:"SSE for closest phonology pattern -- > 3 = blend"',
        )
        self.TrlErr = float()
        self.SetTags(
            "TrlErr",
        ...
        self.SetTags(
            "TrlNameErr",
            'inactive:"+" desc:"1 if trial was error according to name match, 0 if correct -- based on phonology decoding, not SSE"',
        )
        self.TrlSSE = float()
        ...
        self.SetTags(
            "TrlRTCycles",
            'inactive:"+" desc:"current trial\'s number of cycles for phon activity > 0.8"',
        )
        self.EpcSSE = float()
        self.SetTags(
        ...
        self.SetTags(
            "EpcPctNameErr", 'inactive:"+" desc:"last epoch\'s average TrlNameErr"'
        )
    ...
    def Counters(ss, train):
        ...
        if train:
            ...
            return "Run:\t%d\tEpoch:\t%d\tTrial:\t%d\tCycle:\t%d\tName:\t%s\t\t\t" % (
                ...
                ss.Time.Cycle,
                ss.TestEnv.GroupName.Cur + ": " + ss.TestEnv.TrialName.Cur,
            )
    ...
    def OpenPats(ss):
        ...
        ss.OpenPat(
            ...
            "phon_cons.tsv",
            "PhonCons",
            "Phonology patterns -- consonants",
        )
        ss.OpenPat(
            ss.PhonVowelPats,
            "phon_vowel.tsv",
            "PhonVowel",
            "Phonology patterns -- vowels",
        )
    ...
    def ConfigTrnEpcPlot(ss, plt, dt):
        ...
        plt.SetColParams(
            "PctNameErr", eplot.On, eplot.FixMin, 0, eplot.FixMax, 1
        )  # default plot
    ...
    def LogTstEpc(ss, dt):
        ...
        dt.SetCellFloat(
            "PctCor", row, 1 - float(ss.TstErrLog.Rows) / float(minerr.Rows)
        )
    ...
    def ConfigTstRTPlot(ss, plt, dt):
        ...
        plt.SetColParams("Type", eplot.Off, eplot.FixMin, 0, eplot.FloatMax, 0)
        cp = plt.SetColParams(
            "RTCycles:Mean", eplot.On, eplot.FixMin, 0, eplot.FloatMax, 0
        )
