...
type Sim struct {
	...
	// testing patterns based on training data
	ProbePats *etable.Table `view:"no-inline"`

	// nonword testing patterns
	BesnerPats *etable.Table `view:"no-inline"`

	// nonword testing patterns
	GlushkoPats *etable.Table `view:"no-inline"`

	// nonword testing patterns
	TarabanPats *etable.Table `view:"no-inline"`

	// phonology consonant patterns
	PhonConsPats *etable.Table `view:"no-inline"`

	// phonology vowel patterns
	PhonVowelPats *etable.Table `view:"no-inline"`
	...
	// testing trials with errors (aggregating over multiple locations)
	TstErrLog *etable.Table `view:"no-inline"`

	// average RT as a function of type
	TstRTStats *etable.Table `view:"no-inline"`
	...
	// Training environment (frequency-based) -- contains everything about iterating over input / output patterns over training
	...
	// pronunciation from phonology pattern
	...
	// 1 if trial was error according to name match, 0 if correct -- based on phonology decoding, not SSE
	TrlNameErr float64 `inactive:"+"`
	...
	// current trial's number of cycles for phon activity > 0.8
	TrlRTCycles float64 `inactive:"+"`
	...
	// last epoch's average TrlNameErr
	EpcPctNameErr float64 `inactive:"+"`
	...
	// sum to increment as we go through epoch
	SumNameErr float64 `view:"-" inactive:"+"`
...
func (ss *Sim) ConfigEnv() {
	...
	ss.TestEnv.Table = etable.NewIndexView(ss.ProbePats)
...
func (ss *Sim) ConfigTestEnv() {
	...
	case Probe:
		ss.TestEnv.Table = etable.NewIndexView(ss.ProbePats)
	case Besner:
		ss.TestEnv.Table = etable.NewIndexView(ss.BesnerPats)
	case Glushko:
		ss.TestEnv.Table = etable.NewIndexView(ss.GlushkoPats)
	case Taraban:
		ss.TestEnv.Table = etable.NewIndexView(ss.TarabanPats)
	}
	ss.TestEnv.Init(0)
}
...
func (ss *Sim) Counters(train bool) string {
	if train {
		...
		return fmt.Sprintf("Run:\t%d\tEpoch:\t%d\tTrial:\t%d\tCycle:\t%d\tName:\t%s\t\t\t", ss.TrainEnv.Run.Cur, ss.TrainEnv.Epoch.Cur, ss.TestEnv.Trial.Cur, ss.Time.Cycle, ss.TestEnv.GroupName.Cur+": "+ss.TestEnv.TrialName.Cur)
	}
}
...
func (ss *Sim) ConfigGui() *core.Window {
	...
	tbar.AddAction(core.ActOpts{Label: "Open Trained Wts", Icon: "update", Tooltip: "open weights trained for 250 epochs with default params", UpdateFunc: func(act *core.Action) {
		act.SetActiveStateUpdate(!ss.IsRunning)
	}}, win.This(), func(recv, send tree.Ki, sig int64, data interface{}) {
		ss.OpenTrainedWts()
		vp.SetNeedsFullRender()
	...
	tbar.AddAction(core.ActOpts{Label: "README", Icon: "file-markdown", Tooltip: "Opens your browser on the README file that contains instructions for how to run this model."}, win.This(),
		func(recv, send tree.Ki, sig int64, data interface{}) {
			core.OpenURL("https://github.com/CompCogNeuro/sims/blob/master/ch9/ss/README.md")
		})
