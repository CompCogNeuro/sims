...
def LesionCB(recv, send, sig, data):
    LesionDialog(TheSim.vp, TheLesion, "lesion", {}, giv.DlgOpts(Title="Lesion", Prompt="Lesion the network using given type of lesion, and given proportion of neurons (0 < Proportion < 1)"))
    ...
    def LesionNet(ss, les, prop):
        ...
        if les == LesionTypes.AllPartial:
            for ls in range(LesionTypes.OShidden.value, LesionTypes.AllPartial.value):
                for prp in arange(lesStep, 1.0, lesStep):
                    ss.UnLesionNet(net)
