...
def ReadmeCB(recv, send, sig, data):
    core.OpenURL("https://github.com/CompCogNeuro/sims/blob/master/ch9/sg/README.md")
...
class Sim(pyviews.ClassViewObj):
    ...
    def __init__(self):
        ...
        self.SetTags(
            "SentProbeTrlLog", 'view:"no-inline" desc:"probing trial-level log data"'
        )
        self.NounProbeTrlLog = etable.Table()
        self.SetTags(
            "NounProbeTrlLog", 'view:"no-inline" desc:"probing trial-level log data"'
        )
        self.TrnTrlAmbStats = etable.Table()
        self.SetTags(
            "TrnTrlAmbStats", 'view:"no-inline" desc:"aggregate trl stats for last epc"'
        )
        self.TrnTrlQTypStats = etable.Table()
        self.SetTags(
            "TrnTrlQTypStats",
            'view:"no-inline" desc:"aggregate trl stats for last epc"',
        )
        ...
        self.SetTags(
            "SentProbeEnv", 'desc:"Probe environment -- manages iterating over testing"'
        )
        self.NounProbeEnv = ProbeEnv()
        self.SetTags(
            "NounProbeEnv", 'desc:"Probe environment -- manages iterating over testing"'
        )
        ...
        self.LayStatNms = go.Slice_string(
            ["Encode", "EncodeCT", "Gestalt", "GestaltCT", "Decode"]
        )
        self.SetTags(
            "LayStatNms",
        ...
        self.TrlOut = str()
        self.SetTags(
            "TrlOut", 'inactive:"+" desc:"output response(s) output units active > .2"'
        )
        self.TrlPred = str()
    ...
    def ConfigNet(ss, net):
        ...
        el = deep.AddDeepNoTRC2DPy(
            net.AsLeabra(), "Gestalt", 12, 12
        )  # 12x12 def better with full
        gest = el[0]
        gestct = el[1]
        ...
        role.SetRelPos(
            relpos.Rel(
                Rel=relpos.RightOf, Other="EncodeP", YAlign=relpos.Front, Space=4
            )
        )
        fill.SetRelPos(
            relpos.Rel(Rel=relpos.RightOf, Other="Role", YAlign=relpos.Front, Space=4)
        )
        enc.SetRelPos(
            relpos.Rel(
                Rel=relpos.Above, Other="Input", YAlign=relpos.Front, XAlign=relpos.Left
            )
        )
        encct.SetRelPos(
            relpos.Rel(Rel=relpos.RightOf, Other="Encode", YAlign=relpos.Front, Space=2)
        )
        dec.SetRelPos(
            relpos.Rel(
                Rel=relpos.RightOf, Other="EncodeCT", YAlign=relpos.Front, Space=2
            )
        )
        gest.SetRelPos(
            relpos.Rel(
        ...
        gestct.SetRelPos(
            relpos.Rel(
                Rel=relpos.RightOf, Other="Gestalt", YAlign=relpos.Front, Space=2
            )
        )
        ...
        pj = net.ConnectLayers(
            inl, gest, full, emer.Forward
        )  # this is key -- skip encoder
        pj.SetClass("FmInput")
        ...
        net.BidirConnectLayersPy(
            gestct, dec, full
        )  # bidir is essential here to get error signal
        ...
        net.ConnectCtxtToCT(enc, gestct, full).SetClass(
            "CtxtFmInput"
        )  # better than direct from in
    ...
    def Counters(ss, train):
        ...
        if train:
            return (
                "Run:\t%d\tEpoch:\t%d\tSeq:\t%d\tTick:\t%d\tTrial:\t%d\tCycle:\t%d\tName:\t%s\t\t\t"
                % (
                    ss.TrainEnv.Run.Cur,
                    ss.TrainEnv.Epoch.Cur,
                    ss.TrainEnv.Seq.Cur,
                    ss.TrainEnv.Tick.Cur,
    ...
    def TrialStats(ss, accum):
        ...
        for li, lnm in enumerate(ss.StatLayNms):
            ...
            if lnm == "Filler":
                ss.TrlOut = ", ".join(ss.ActiveUnitNames(lnm, ss.TrainEnv.Fillers, 0.2))
            if lnm == "EncodeP":
                ss.TrlPred = ", ".join(ss.ActiveUnitNames(lnm, ss.TrainEnv.Words, 0.2))
    ...
    def ConfigTrnTrlPlot(ss, plt, dt):
        ...
        for lnm in ss.StatNms:
            ...
            plt.SetColParams(
                lnm + "AvgSSE", eplot.Off, eplot.FixMin, 0, eplot.FixMax, 1
            )
            plt.SetColParams(
                lnm + "CosDiff", eplot.On, eplot.FixMin, 0, eplot.FixMax, 1
            )
        return plt
    ...
    def ConfigTrnEpcPlot(ss, plt, dt):
        ...
        for lnm in ss.StatNms:
            ...
            plt.SetColParams(
                lnm + "AvgSSE", eplot.Off, eplot.FixMin, 0, eplot.FloatMax, 0
            )
            plt.SetColParams(
                lnm + "PctErr", eplot.On, eplot.FixMin, 0, eplot.FixMax, 1
            )  # default plot
            plt.SetColParams(
                lnm + "PctCor", eplot.Off, eplot.FixMin, 0, eplot.FixMax, 1
            )
            plt.SetColParams(
                lnm + "CosDiff", eplot.Off, eplot.FixMin, 0, eplot.FixMax, 1
            )

        cols = go.Slice_string(["Err", "SSE", "CosDiff"])
        for cl in cols:
            plt.SetColParams(
                "UnAmbFill" + cl, eplot.Off, eplot.FixMin, 0, eplot.FixMax, 1
            )
            plt.SetColParams(
                "AmbFill" + cl, eplot.Off, eplot.FixMin, 0, eplot.FixMax, 1
            )
            plt.SetColParams(
                "CurQFill" + cl, (cl == "Err"), eplot.FixMin, 0, eplot.FixMax, 1
            )
            plt.SetColParams(
                "RevQFill" + cl, (cl == "Err"), eplot.FixMin, 0, eplot.FixMax, 1
            )
        ...
        for lnm in ss.LayStatNms:
            ...
            plt.SetColParams(
                lnm + " Hog", eplot.Off, eplot.FixMin, 0, eplot.FixMax, 0.5
            )
            plt.SetColParams(
                lnm + " Dead", eplot.Off, eplot.FixMin, 0, eplot.FixMax, 0.5
            )
        return plt
    ...
    def ProbeClusterPlot(ss):
        ...
        """
        stix = etable.NewIndexView(ss.SentProbeTrlLog)
        ...
        ntix = etable.NewIndexView(ss.NounProbeTrlLog)
    ...
    def ConfigGui(ss):
        ...
        win = core.NewMainWindow("SG", "Sentence Gestalt", width, height)
        ss.Win = win
        ...
        tbar.AddAction(
            core.ActOpts(
                Label="Step Seq",
                Icon="fast-fwd",
                Tooltip="Advances one sequence (sentence) at a time.",
                UpdateFunc=UpdateFuncNotRunning,
            ),
            recv,
            StepSeqCB,
        ...
        tbar.AddAction(
            core.ActOpts(
                Label="Init Test",
                Icon="update",
                Tooltip="Initialize to start of testing items.",
                UpdateFunc=UpdateFuncNotRunning,
            ),
            recv,
            InitTestCB,
        ...
        tbar.AddAction(
            core.ActOpts(
                ...
                Icon="reset",
                Tooltip="Resets the testing trial log, so it is easier to read",
            ),
            recv,
            ResetTstTrlLogCB,
        )
