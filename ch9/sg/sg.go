...
type Sim struct {
	...
	Net *deep.Network `view:"no-inline"`
	...
	// probing trial-level log data
	SentProbeTrlLog *etable.Table `view:"no-inline"`
	// probing trial-level log data
	NounProbeTrlLog *etable.Table `view:"no-inline"`
	// aggregate trl stats for last epc
	TrnTrlAmbStats *etable.Table `view:"no-inline"`
	// aggregate trl stats for last epc
	TrnTrlQTypStats *etable.Table `view:"no-inline"`
	...
	TestEnv SentGenEnv
	// Probe environment -- manages iterating over testing
	SentProbeEnv SentGenEnv
	// Probe environment -- manages iterating over testing
	NounProbeEnv ProbeEnv
	...
	// stat layers
	StatLayNms []string `view:"-"`
	// stat short names
	StatNms []string `view:"-"`
	// layers to probe
	ProbeNms []string `view:"-"`

	// output response(s) output units active > .2
	TrlOut string `inactive:"+"`
	// predicted word(s) active > .2
	TrlPred string `inactive:"+"`
	// 1 if trial was error, 0 if correct -- based on SSE = 0 (subject to .5 unit-wise tolerance)
	TrlErr [2]float64 `inactive:"+"`
	// current trial's sum squared error
	TrlSSE [2]float64 `inactive:"+"`
	// current trial's average sum squared error
	TrlAvgSSE [2]float64 `inactive:"+"`
	// current trial's cosine difference
	TrlCosDiff [2]float64 `inactive:"+"`
	// last epoch's total sum squared error
	EpcSSE [2]float64 `inactive:"+"`
	// last epoch's average sum squared error (average over trials, and over units within layer)
	EpcAvgSSE [2]float64 `inactive:"+"`
	// last epoch's average TrlErr
	EpcPctErr [2]float64 `inactive:"+"`
	// 1 - last epoch's average TrlErr
	EpcPctCor [2]float64 `inactive:"+"`
	...
	// number of each stat
	SumN [2]float64 `view:"-" inactive:"+"`
	// sum to increment as we go through epoch
	SumErr [2]float64 `view:"-" inactive:"+"`
	// sum to increment as we go through epoch
	SumSSE [2]float64 `view:"-" inactive:"+"`
	// sum to increment as we go through epoch
	SumAvgSSE [2]float64 `view:"-" inactive:"+"`
	// sum to increment as we go through epoch
	SumCosDiff [2]float64 `view:"-" inactive:"+"`
	// main GUI window
	...
	// the probe cluster plot
	SentProbeClustPlot *eplot.Plot2D `view:"-"`
	// the probe cluster plot
	NounProbeClustPlot *eplot.Plot2D `view:"-"`
	// the run plot
...
func (ss *Sim) LogEpcStats(dt *etable.Table, trlog *etable.Table) {
	...
	// noambtix := etable.NewIndexView(trlog)
...
func (ss *Sim) ClustPlot(plt *eplot.Plot2D, ix *etable.IndexView, colNm, lblNm string, dfunc clust.DistFunc) {
...
func (ss *Sim) ConfigGui() *core.Window {
	...
	tbar.AddAction(core.ActOpts{Label: "Step Seq", Icon: "fast-fwd", Tooltip: "Advances one sequence (sentence) at a time.", UpdateFunc: func(act *core.Action) {
		act.SetActiveStateUpdate(!ss.IsRunning)
	}}, win.This(), func(recv, send tree.Ki, sig int64, data interface{}) {
		if !ss.IsRunning {
			ss.IsRunning = true
	...
	tbar.AddAction(core.ActOpts{Label: "Open Weights", Icon: "update", Tooltip: "Open trained weights ", UpdateFunc: func(act *core.Action) {
		act.SetActiveStateUpdate(!ss.IsRunning)
	}}, win.This(), func(recv, send tree.Ki, sig int64, data interface{}) {
		ss.OpenWts()
	})

	tbar.AddAction(core.ActOpts{Label: "Init Test", Icon: "update", Tooltip: "Initialize to start of testing items.", UpdateFunc: func(act *core.Action) {
		act.SetActiveStateUpdate(!ss.IsRunning)
	}}, win.This(), func(recv, send tree.Ki, sig int64, data interface{}) {
		ss.InitTest()
		vp.SetNeedsFullRender()
	...
	tbar.AddAction(core.ActOpts{Label: "Test Seq", Icon: "fast-fwd", Tooltip: "Advances one sequence (sentence) at a time.", UpdateFunc: func(act *core.Action) {
		act.SetActiveStateUpdate(!ss.IsRunning)
	}}, win.This(), func(recv, send tree.Ki, sig int64, data interface{}) {
		if !ss.IsRunning {
			ss.IsRunning = true
	...
	tbar.AddAction(core.ActOpts{Label: "Reset TstTrlLog", Icon: "update", Tooltip: "Reset the Testing trial log, so it is easier to read"}, win.This(),
		func(recv, send tree.Ki, sig int64, data interface{}) {
			ss.TstTrlLog.SetNumRows(0)
			ss.TstTrlPlot.Update()
		})

	tbar.AddAction(core.ActOpts{Label: "Probe All", Icon: "fast-fwd", Tooltip: "probe inputs.", UpdateFunc: func(act *core.Action) {
		act.SetActiveStateUpdate(!ss.IsRunning)
	}}, win.This(), func(recv, send tree.Ki, sig int64, data interface{}) {
		if !ss.IsRunning {
			ss.ProbeAll()
