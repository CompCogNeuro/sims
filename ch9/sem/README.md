...
* Compare several other words that the network should know about from reading this textbook (tip: `TrainEnv` and then on `Words` to see a list of all the words, and scroll through that to see what words are in the valid list (these are words with frequency greater than 5, and not purely syntactic).
