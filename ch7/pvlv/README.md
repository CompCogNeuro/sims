...
<!-- Back to [CCNBook/Sims/All](/CCNBook/Sims/All "wikilink") or [Motor
Control and Reinforcement Learning Chapter](/CCNBook/Motor "wikilink"). -->
...
model.
<!-- [RL](/CCNBook/Sims/Motor/RL "wikilink") -->
...
<!-- -   Note: The PVLV project uses a control framework called *param sets*
    which affords broad flexibility in exploring many different variants
    of the training environment and network configuration. Individual
    param_sets store separate sets of parameters for each specific
    variant. In particular, param sets allow for the concatenation of
    multiple different training environments by assigning them in
    sequence using seq_params param sets. Each named seq_params item
    specifies a sequence of one or more run_params items, which can
    then run seamlessly until the full sequence is completed. -->
