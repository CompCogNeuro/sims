...
func (ss *Sim) OpenPatterns() {
	ss.ABPatterns.SetMetaData("name", "AB")
	ss.ABPatterns.SetMetaData("desc", "AB Training Patterns")
	errors.Log(ss.ABPatterns.OpenFS(content, "ab_pats.tsv", table.Tab))
	col := table.InsertColumn[string](ss.ABPatterns, "Group", 1)
	for i := range col.DimSize(0) {
		col.SetString1D(i, "AB")
	}
	ss.ACPatterns.SetMetaData("name", "AC")
	ss.ACPatterns.SetMetaData("desc", "AC Training Patterns")
	errors.Log(ss.ACPatterns.OpenFS(content, "ac_pats.tsv", table.Tab))
	col = table.InsertColumn[string](ss.ACPatterns, "Group", 1)
	for i := range col.DimSize(0) {
		col.SetString1D(i, "AC")
	}
