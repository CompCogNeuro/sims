...
type Sim struct {
	...
	// click to see the full face testing input patterns to use
	Pats *etable.Table `view:"no-inline"`
	// click to see the partial face testing input patterns to use
	PartPats *etable.Table `view:"no-inline"`
	...
	PrjnTable *etable.Table `view:"no-inline"`
	...
	ClustFaces *eplot.Plot2D `view:"no-inline"`
	// cluster plot of emotions
	ClustEmote *eplot.Plot2D `view:"no-inline"`
	// cluster plot of genders
	ClustGend *eplot.Plot2D `view:"no-inline"`
	// cluster plot of identity
	ClustIdent *eplot.Plot2D `view:"no-inline"`
	// random projection plot
	PrjnRandom *eplot.Plot2D `view:"no-inline"`
	// projection plot of emotions & gender
	PrjnEmoteGend *eplot.Plot2D `view:"no-inline"`
...
func (ss *Sim) InitWts(net *leabra.Network) {
	net.InitWts()
	ab, err := content.ReadFile("faces.wts")
	if err != nil {
		log.Println(err)
	}
