...
def SetInputCB(recv, send, sig, data):
    ...
    gi.ChoiceDialog(
        ...
        go.Slice_string(["Bottom-up", "Top-down"]),
        win,
        SetInputCB2,
    )
...
def SetPatsCB(recv, send, sig, data):
    ...
    gi.ChoiceDialog(
        ...
        go.Slice_string(["Full Faces", "Partial Faces"]),
        win,
        SetPatsCB2,
    )
...
class Sim(pygiv.ClassViewObj):
    ...
    def __init__(self):
        ...
        self.SetTags(
            "Pats",
            'view:"no-inline" desc:"click to see the full face testing input patterns to use"',
        )
        self.PartPats = etable.Table()
        self.SetTags(
            "PartPats",
            'view:"no-inline" desc:"click to see the partial face testing input patterns to use"',
        )
        ...
        self.SetTags(
            "PrjnEmoteGend",
            'view:"no-inline" desc:"projection plot of emotions & gender"',
        )
    ...
    def PrjnPlot(ss):
        ...
        for i in range(256):
            rvec0.Values[i] = 0.15 * (2 * rand.Float32() - 1)
            rvec1.Values[i] = 0.15 * (2 * rand.Float32() - 1)
        ...
        for r in range(nr):
            emote = 0.5 * tst.CellTensorFloat1D(
                "Emotion", r, 0
            ) + -0.5 * tst.CellTensorFloat1D("Emotion", r, 1)
            emote += 0.1 * (2 * rand.Float64() - 1)

            gend = 0.5 * tst.CellTensorFloat1D(
                "Gender", r, 0
            ) + -0.5 * tst.CellTensorFloat1D("Gender", r, 1)
            gend += 0.1 * (2 * rand.Float64() - 1)  # some jitter so labels are readable
    ...
    def ConfigTstTrlPlot(ss, plt, dt):
        ...
        plt.SetColParams(
            "Gender", eplot.On, eplot.FixMin, 0, eplot.FixMax, 1
        )  # display
        return plt

    def ConfigNetView(ss, nv):
        labs = go.Slice_string(
            ["happy sad", "female  male", "Albt Bett Lisa Mrk Wnd Zane"]
        )
