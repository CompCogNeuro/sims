...
* To see this effect, set `Delay` back to `Delay3` and then reduce the `RecurrentWt` parameter to .1.  `Init`, `Train`, and look at the activations of the units in the 'B' choice trial. Then compare this with the case with of .4.  It can be easier to see the difference in the `TrnTrlPlot` -- click on the `Reach` button to pull up a dialog to configure the display of that line, and change the `TensorIdx` between 0 and 1 -- the 0 is the `A` unit, and 1 is the `B` unit.
