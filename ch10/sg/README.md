...
* Set the run mode to `Test` instead of `Train`, and do `Init` and then `Step Trial` to see the start of the first test sentence, which is the special `start` indicator. Do `Step Trial` again to see the first word in the sentence.
