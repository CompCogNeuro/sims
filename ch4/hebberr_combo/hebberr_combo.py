...
def ReadmeCB(recv, send, sig, data):
    core.OpenURL(
        "https://github.com/CompCogNeuro/sims/blob/master/ch4/hebberr_combo/README.md"
    )
...
class Sim(pyviews.ClassViewObj):
    ...
    def UpdateEnv(ss):
        if ss.Pats == PatsType.Easy:
            ...
            splits = split.Permuted(
                all, go.Slice_float64([0.9, 0.1]), go.Slice_string(["Train", "Test"])
            )
                ...
                if ss.ViewOn:
                    if viewUpdate == leabra.Cycle:
                        if cyc != ss.Time.CycPerQtr - 1:  # will be updated by quarter
                            ss.UpdateView(train)
                    elif viewUpdate == leabra.FastSpike:
                        if (cyc + 1) % 10 == 0:
                            ss.UpdateView(train)
    ...
    def LogRun(ss, dt):
        ...
        if nlast > epcix.Len() - 1:
            nlast = epcix.Len() - 1
        epcix.Indexes = go.Slice_int(epcix.Indexes[epcix.Len() - nlast :])
    ...
    def ConfigGui(ss):
        ...
        core.SetAppAbout(
            'shows how XCal hebbian learning in shallower layers of a network can aid an error driven learning network to generalize to unseen combinations of patterns. See <a href="https://github.com/CompCogNeuro/sims/blob/master/ch4/hebberr_combo/README.md">README.md on GitHub</a>.</p>'
        )

        win = core.NewMainWindow(
            "hebberr_combo", "Hebbian+Error Driven Learning", width, height
        )
        ss.Win = win
