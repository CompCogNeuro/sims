...
type Sim struct {
	...
	Easy *etable.Table `view:"no-inline"`
	// hard training patterns -- require error-driven
	Hard *etable.Table `view:"no-inline"`
	// impossible training patterns -- require error-driven + hidden layer
	Impossible *etable.Table `view:"no-inline"`
...
func (ss *Sim) ConfigEnv() {
	...
	ss.TrainEnv.Table = etable.NewIndexView(ss.Easy)
	ss.TrainEnv.Validate()
	ss.TrainEnv.Run.Max = ss.MaxRuns // note: we are not setting epoch max -- do that manually

	ss.TestEnv.Nm = "TestEnv"
	ss.TestEnv.Dsc = "testing params and state"
	ss.TestEnv.Table = etable.NewIndexView(ss.Easy)
...
func (ss *Sim) UpdateEnv() {
	...
	case Easy:
		...
		ss.TestEnv.Table = etable.NewIndexView(ss.Easy)
	case Hard:
		ss.TrainEnv.Table = etable.NewIndexView(ss.Hard)
		ss.TestEnv.Table = etable.NewIndexView(ss.Hard)
	case Impossible:
		ss.TrainEnv.Table = etable.NewIndexView(ss.Impossible)
		ss.TestEnv.Table = etable.NewIndexView(ss.Impossible)
	case Lines2:
		all := etable.NewIndexView(ss.Lines2)
		splits, _ := split.Permuted(all, []float64{.9, .1}, []string{"Train", "Test"})
		ss.TrainEnv.Table = splits.Splits[0]
		ss.TestEnv.Table = splits.Splits[1]

		//	ss.TrainEnv.Table =  splits.Splits[0]//etable.NewIndexView(ss.Lines2)
		//	ss.TestEnv.Table =  splits.Splits[1]//etable.NewIndexView(ss.Lines2)
...
func (ss *Sim) ConfigGui() *core.Window {
	...
	core.SetAppName("hebberr_combo")
	core.SetAppAbout(`shows how XCal hebbian learning in shallower layers of a network can aid an error driven learning network to generalize to unseen combinations of patterns.
 See <a href="https://github.com/CompCogNeuro/sims/blob/master/ch4/hebberr_combo/README.md">README.md on GitHub</a>.</p>`)

	win := core.NewMainWindow("hebberr_combo", "Hebbian+Error Driven Learning", width, height)
	ss.Win = win
	...
	tbar.AddAction(core.ActOpts{Label: "README", Icon: "file-markdown", Tooltip: "Opens your browser on the README file that contains instructions for how to run this model."}, win.This(),
		func(recv, send tree.Node, sig int64, data interface{}) {
			core.OpenURL("https://github.com/CompCogNeuro/sims/blob/master/ch4/hebberr_combo/README.md")
		})
