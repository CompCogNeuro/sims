...
* Click on `Stats` in the left panel, then click on the `Sim Mats`, and then on button next to `HiddenRel`, to bring up this similarity matrix for the Hidden layer, with patterns labeled and sorted according to the type of relationship encoded.  This sorting is key to making the patterns of similarity related to this relationship factor evident in the similarity matrix.
...
* Next, click on the `HiddenAgent`, which shows the hidden unit activation distances organized by the agent role. Look back and forth between this and the previous relationship-organized plot to compare them.  You can hold down the `Shift` key to pull up `Sim Mats` in separate windows.
...
* Click on the `HiddenRelClust` tab to see the cluster plot of the Hidden activations labeled by relationships.
...
* Click on the `HiddenRelPCA` tab to see the PCA plot for the Hidden layer organized by relations.
