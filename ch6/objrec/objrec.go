...
/*
objrec shows how XCal error driven learning can train a hidden layer to solve problems that are otherwise impossible for a simple two layer network (as we saw in the Pattern Associator exploration, which should be completed first before doing this one).
*/
package main
...
func (ss *Sim) LogTstTrl(dt *etable.Table) {
	epc := ss.TrainEnv.Epoch.Prv // this is triggered by increment so use previous value
	inp := ss.Net.LayerByName("V1").(*leabra.Layer)
	out := ss.Net.LayerByName("Output").(*leabra.Layer)
...
func (ss *Sim) ConfigGui() *gi.Window {
	...
	gi.SetAppAbout(`shows how XCal error driven learning can train a hidden layer to solve problems that are otherwise impossible for a simple two layer network (as we saw in the Pattern Associator exploration, which should be completed first before doing this one). See <a href="href="https://github.com/CompCogNeuro/sims/ch6/objrec/README.md">README.md on GitHub</a>.</p>`)

	win := gi.NewWindow2D("objrec", "Error Driven Hidden Layer Learning", width, height, true)
	ss.Win = win
