...
func (ss *Sim) LogTstEpc(dt *etable.Table) {
	...
	_, err := split.AggTry(spl, "Error", agg.AggMean)
	if err != nil {
		log.Println(err)
	}
	objs := spl.AggsToTable(false)
	no := objs.Rows
	dt.SetNumRows(no)
	for i := 0; i < no; i++ {
