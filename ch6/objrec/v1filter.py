...
class Vis(pygiv.ClassViewObj):
    ...
    def __init__(self):
        ...
        self.SetTags("V1sGeom", 'inactive:"+" view:"inline" desc:"geometry of input, output for V1 simple-cell processing"')
        self.V1sNeighInhib = kwta.NeighInhib()
        self.SetTags("V1sNeighInhib", 'desc:"neighborhood inhibition for V1s -- each unit gets inhibition from same feature in nearest orthogonal neighbors -- reduces redundancy of feature code"')
        ...
        self.SetTags("V1sGaborTsr", 'view:"no-inline" desc:"V1 simple gabor filter tensor"')
        ...
        self.SetTags("V1sTsr", 'view:"no-inline" desc:"V1 simple gabor filter output tensor"')
        self.V1sExtGiTsr = etensor.Float32()
        self.SetTags("V1sExtGiTsr", 'view:"no-inline" desc:"V1 simple extra Gi from neighbor inhibition tensor"')
        self.V1sKwtaTsr = etensor.Float32()
        self.SetTags("V1sKwtaTsr", 'view:"no-inline" desc:"V1 simple gabor filter output, kwta output tensor"')
        self.V1sPoolTsr = etensor.Float32()
        self.SetTags("V1sPoolTsr", 'view:"no-inline" desc:"V1 simple gabor filter output, max-pooled 2x2 of V1sKwta tensor"')
        self.V1sUnPoolTsr = etensor.Float32()
        self.SetTags("V1sUnPoolTsr", 'view:"no-inline" desc:"V1 simple gabor filter output, un-max-pooled 2x2 of V1sPool tensor"')
        self.V1sAngOnlyTsr = etensor.Float32()
        self.SetTags("V1sAngOnlyTsr", 'view:"no-inline" desc:"V1 simple gabor filter output, angle-only features tensor"')
        self.V1sAngPoolTsr = etensor.Float32()
        self.SetTags("V1sAngPoolTsr", 'view:"no-inline" desc:"V1 simple gabor filter output, max-pooled 2x2 of AngOnly tensor"')
        self.V1cLenSumTsr = etensor.Float32()
        self.SetTags("V1cLenSumTsr", 'view:"no-inline" desc:"V1 complex length sum filter output tensor"')
        self.V1cEndStopTsr = etensor.Float32()
        self.SetTags("V1cEndStopTsr", 'view:"no-inline" desc:"V1 complex end stop filter output tensor"')
        self.V1AllTsr = etensor.Float32()
        self.SetTags("V1AllTsr", 'view:"no-inline" desc:"Combined V1 output tensor with V1s simple as first two rows, then length sum, then end stops = 5 rows total"')
        self.V1sInhibs = fffb.Inhibs()
        self.SetTags("V1sInhibs", 'view:"no-inline" desc:"inhibition values for V1s KWTA"')
    ...
    def SetImage(vi, img):
        ...
        if isz != vi.ImgSize:
            vi.Img = gi.ImageResize(vi.Img, vi.ImgSize.X, vi.ImgSize.Y)
    ...
    def V1All(vi):
        ...
        if not etensor.EqualInts(oshp, vi.V1AllTsr.Shp):
            vi.V1AllTsr.SetShape(oshp, go.nil, go.Slice_string(["Y", "X", "Polarity", "Angle"]))
