...
* Select `Act` in the Network window, then switch from `Train` to `Test` mode, and do `Step Trial` in the toolbar, and continue to `Step` Trial through the next 3 events, and note the relationship in each case to the weight-based receptive fields shown in the V1 RFs plot.
